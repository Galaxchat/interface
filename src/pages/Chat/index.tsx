...
import { useERC721Contract, useContract } from '../../hooks/useContract'
...
export default function Chat({ history }: RouteComponentProps) {
  ...
  const erc721Contract =  useERC721Contract(enterQuery)
  ...
  const handleEnter = useCallback((e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (searchValue){
        setEnterQuery(searchValue)
      }

      if (erc721Contract) {
        setQueryResult(erc721Contract)
      }
      console.log(erc721Contract)
      console.log("queryResult:",queryResult)
    }
  },[queryResult, enterQuery]);
